...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
  ...
  if(!empty($phone_number) && !empty($location) && !empty($house_address) && !empty($collection_date) && !is_numeric($location))
  {
      ...
      if($location === "Tigoni"){
        header("Location:../order_payment/payment.php?error=Collection for Tigoni is Kshs.350");
	    exit();
      }else if($location === "Kabuku"){
        header("Location:../order_payment/payment.php?error= Collection for Kabuku is Kshs.200");
	    exit();
      }else if($location === "Ngecha"){
        header("Location:../order_payment/payment.php?error= Collection for Ngecha is Kshs.300");
	    exit();
      }
  }else
  {
	  header("Location:order.php?error=Invalid Location name");
	  exit();
  }
}
                ...
                <div class="card mt-5">
                    ...
                    <div class="card-body">
                        ...
                        <form action="" method="POST">
							...
							<a href="../panel/user.php" class="btn btn-primary">HOME</a>
